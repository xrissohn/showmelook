...
export const PlanCard = ({
  ...
  const planDisplayName = language === 'en' ? config.name : config.nameKo;
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-6">
        ...
        <h3 className="text-xl font-bold font-korean">{planDisplayName}</h3>
        <p className="text-sm text-muted-foreground font-korean">{config.name}</p>
      </div>

      <div className="text-center mb-6">
        {price === 0 ? (
          <div className="text-3xl font-bold">{t('subscription.free')}</div>
        ) : (
          <>
            <div className="text-3xl font-bold">
              {formatPrice(isYearly ? monthlyEquivalent : price)}
              <span className="text-base font-normal text-muted-foreground">{t('subscription.perMonth')}</span>
            </div>
            {isYearly && price > 0 && (
              <div className="mt-1 space-y-1">
                <div className="text-sm text-muted-foreground">
                  {t('subscription.yearlyTotal').replace('{price}', formatPrice(price))}
                </div>
                <Badge variant="secondary" className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">
                  {t('subscription.savings').replace('{amount}', formatPrice(savings))}
                </Badge>
              </div>
            )}
          </>
      ...
      >
        {isCurrentPlan ? t('subscription.currentPlan') : plan === 'free' ? t('subscription.startFree') : t('subscription.upgrade')}
      </Button>
    </div>
  );
};
