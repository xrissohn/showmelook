...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // GET = legacy email link. Just redirect to the React app which will POST.
    if (req.method === 'GET') {
      const url = new URL(req.url);
      const token = url.searchParams.get('token') || '';
      const choice = url.searchParams.get('choice') || '';
      const target = new URL('https://showmelook.com/survey/shomi');
      if (token) target.searchParams.set('token', token);
      if (choice) target.searchParams.set('choice', choice);
