// style-recommend v9.0 - 사진 평가 모드 + Lovable AI Gateway 통일
...
2. 필수 아이템 (requiredItems):
   ...
   - 일반 상황: ["상의", "하의"] 필수
   - 야외/겨울: ["아우터"] 고려
   - ⚠️ 사용자가 "원피스", "드레스"를 요청하면 requiredItems에 "원피스"를 반드시 포함하고, "상의"와 "하의"는 제외하세요
...
1. **한 줄 오프닝** (위트있는 자신감 한 줄)
2. **코디 포인트** (선택한 아이템들의 핵심 조합 이유 2~3줄)
   - 색상 조화, 실루엣, 소재 매칭 등
   - ⚠️ 반드시 선택한 상품의 브랜드명만 언급!
3. **스타일링 팁** (프로 노하우 1줄)
4. **마무리 한 줄** (자신감 넘치는 클로징)
